...
class ManualTestSuite:
    ...
    async def test_challenge_system(self):
        ...
        try:
            if 'players' not in self.test_data or 'games' not in self.test_data:
                self.print_error("Missing test data for challenge tests")
                return False
            
            players = self.test_data['players']
            games = self.test_data['games']
            
            if len(players) < 2 or len(games) < 1:
            ...
            self.print_info(f"Creating challenge: {challenger.username} vs {challenged.username} in {game.name}")
            ...
            self.print_success(f"Created challenge ID: {challenge.id}")
            self.test_data['challenge'] = challenge
            
            # Test challenge retrieval
            retrieved_challenge = await self.db.get_challenge_by_id(challenge.id)
            if retrieved_challenge:
                self.print_success("Challenge retrieved successfully")
                self.print_info(f"Challenge details: {retrieved_challenge.challenger.username} vs {retrieved_challenge.challenged.username}")
            else:
                self.print_error("Failed to retrieve challenge")
            
            # Test challenge status update
        ...
        try:
            ...
            if 'challenge' in self.test_data:
                ...
                if challenge:
                    ...
                    if challenge.challenger and challenge.challenged and challenge.game:
